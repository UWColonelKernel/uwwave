import $ from 'jquery'
import { addLocalStorageListener, clearLocalStorage, getLocalStorage, setLocalStorage } from '../common/storage'
import { JOB_DATA_IDENTIFIER } from '../common/job'
...
function updateJobCount() {
    getLocalStorage(null).then((results) => {
        const keys = Object.keys(results).filter((key) => key.indexOf(JOB_DATA_IDENTIFIER) !== -1);
